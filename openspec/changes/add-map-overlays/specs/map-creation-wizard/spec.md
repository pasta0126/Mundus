...
(see `map-layers`) as an overlay, consistent with every other UI element.

#### Scenario: The legend is visible alongside the map
- **WHEN** a map is rendered with at least one icon layer visible
- **THEN** the legend overlay is shown alongside the canvas
