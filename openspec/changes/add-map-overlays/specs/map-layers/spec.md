...
only the icons belonging to currently visible points-of-interest categories
(and the compass rose, when visible); an icon whose layer is hidden SHALL
NOT appear in the legend.

#### Scenario: Hiding a category removes it from the legend
- **WHEN** a user hides the "sea legends" points-of-interest category
- **THEN** the legend no longer lists that category's icons
