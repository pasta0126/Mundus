...
86 icons, cut from one artwork sheet (the original is kept in
`frontend/src/assets/poi/source/`), one transparent PNG each in
`frontend/src/assets/poi/<id>.png`. Ids are the catalog's; every id has art
and no art lacks an id, enforced by tests. Categories (layers): relief &
geology, nature & wildlife, sea & islands, settlements, monuments & ruins,
legends & mysteries. Icons name themselves on hover (a tooltip fed by the
catalog's label and description), so the legend only lists biomes.
